...
def direction_change(x,y,file,angle_threshold=90): 
    ...
    def in_hitbox(x,y,loc_center,hitbox_size=100): ## FILL THIS FUNCTION ##  This function return if x and y are within loc hitbox
        hitbox_limit = hitbox_size/2
        x_inside = (x>loc_center[0]-hitbox_limit) and (x < loc_center[0]+hitbox_limit)
        y_inside = (y>loc_center[1]-hitbox_limit) and (y < loc_center[1]+hitbox_limit)
    ...
    for frame in range(len(x)-3) :
        ...
        if direction_change :
            for loc in locations:
                ...
                current_res= (res["trial_stage"] == stage) & (res["loc"] == loc)
                if in_hitbox(x[frame+1],y[frame+1],locations[loc]): 
